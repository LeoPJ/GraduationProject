...
def write_file(str):
    ...
    :return: 无
    """
    writefile = open("./out/dt2_users_out.txt", 'a+',encoding='utf-8')
    writefile.write(str + '\n')
    writefile.close()
    # return str
...
if __name__ == '__main__':
    ...
    with open('data/dt2_users.json',encoding='utf-8') as f:
        dicts = json.load(f)
    dict = {}
    for d in dicts:
        dict[d["no"]] = d['ONE_HOT']
    ...
    # ~ np.savetxt("dt2xz_data.txt", W,fmt="%f",delimiter=",")
    # ~ np.save("dt2xzdata.npy",W) 
    
    W=np.load("dt2xzdata.npy")
    print(W)
    # print(W[0][0], W[2][1], W[1][2], W[2][2])
    

    af = AffinityPropagation(max_iter=10000,affinity='precomputed').fit(W)
    #af_data=open("af.txt",'w+')
    print(af)
    #print(af,file=af_data)
    
    cluster_centers_indices = af.cluster_centers_indices_
    #cluster_centers_data=open("cluster_centers_data.txt",'w+')
    print(cluster_centers_indices)
    #print(cluster_centers_indices,file=cluster_centers_data)
    
    labels = af.labels_
    #labels_data=open("labels_data.txt",'w+')
    print(labels)
    #print(labels,file=labels_data)
    ...
    write_file("*********【分{}类】*********".format(n_clusters_))
    for i in range(n_clusters_):
        cluster_index = find_index(list(labels), i)
        tem_list = []
        for index in cluster_index:
            tem_list.append(dict[index])
    ...
    # center_index = np.array(cluster_centers_indices)
    # np.savetxt("./output3/center_index.txt", center_index)
    #
    # index_for_all = np.array(labels)
    # np.savetxt("./output3/index_for_all.txt", index_for_all)
